...
改坏后满足任一条就算杀死：
  ...
  * **非 0 退出码** —— 带断言的例子（`_shared.Checks`）改坏一条不变量
...
第 08 章 a（元数据过滤的租户隔离）、第 10 章 b（派活的失败隔离）、
第 11 章 a/b（评测判卷、上下文裁剪）、第 16 章实验 4（工具白名单 / 路径范围）、
...
MUTATIONS: list[Mutation] = [
    ...
    # ── 第 10 / 11 章：派活隔离 / 判卷 / 裁剪 ─────────────────────
    Mutation(
        id="10b·失败不隔离",
        file="10b_supervisor_lab.py",
        invariant="聚合不过滤状态 → 崩掉的 worker 连同 None 数据混进简报正文",
        old=(
            '        if r["status"] == "ok":\n'
            '            brief["正文"].append((r["worker"], r["data"]))'
        ),
        new=(
            '        if True:  # 变异：不再过滤失败 worker\n'
            '            brief["正文"].append((r["worker"], r["data"]))'
        ),
        expect_appear="失败 worker 被隔离",
    ),
    Mutation(
        id="11a·判卷失守",
        file="11a_eval_lab.py",
        invariant="判卷恒真 → 改坏 prompt 的 v1 也满分，评测集测不出任何退化",
        old="        case_ok = all(ok for _, ok in checks)",
        new="        case_ok = True  # 变异：判卷失守，人人满分",
        expect_appear="v1 应当恰好通过 2/4",
    ),
    Mutation(
        id="11b·裁剪失效",
        file="11b_context_budget_lab.py",
        invariant="裁剪不裁 → 上下文不变小，'裁剪丢早期事实'的取舍演示失效",
        old="    kept = TURNS[-RECENT_N:]                       # 裁剪：只留最近 N 轮",
        new="    kept = TURNS  # 变异：裁剪失效，全量保留",
        expect_appear="裁剪版确实丢了早期事实",
    ),
