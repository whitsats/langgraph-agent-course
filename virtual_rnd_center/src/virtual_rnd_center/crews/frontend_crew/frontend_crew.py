"""
frontend_crew.py — Frontend Specialized Crew v2.0
==================================================
Changes from v1.0:
- Fixed import: use virtual_rnd_center.tools.file_read_tool (not crewai_tools)
- P0-3: ScopeGuardTool added to architect, frontend_dev.
        fe_code_task gets scope_guardrail Guardrail.
...
@CrewBase
class FrontendCrew:
    """Frontend Specialized Crew — UI/UX Layer (v2.0)."""
    ...
    @task
    def fe_design_task(self) -> Task:
        return Task(config=self.tasks_config["fe_design_task"])  # type: ignore[index]

    @task
    def fe_planning_task(self) -> Task:
        return Task(
            config=self.tasks_config["fe_planning_task"],  # type: ignore[index]
            human_input=True,
        )

    @task
    def fe_code_task(self) -> Task:
        # P0-3: scope guardrail; P1-C: context chain
        return Task(
            config=self.tasks_config["fe_code_task"],  # type: ignore[index]
            context=[self.fe_planning_task()],
            guardrail=scope_guardrail,
            guardrail_max_retries=2,
        )

    @task
    def fe_qa_task(self) -> Task:
        return Task(
            config=self.tasks_config["fe_qa_task"],  # type: ignore[index]
            human_input=True,
        )
