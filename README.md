...
1. **`tutorial/`** ← 主入口（讲义 20 章：正文 14 章 + 6 篇附录）。从 [`tutorial/00-准备工作.md`](tutorial/00-准备工作.md) 开始，
   ...
   **每章配一个能直接跑的例子**，读一章跑一个：
   ...
   > ✅ 实测状态（2026-09-25）：离线 **11/11**、变异检查 **23/23**、在线全量 **10/11**——
   > 唯一失败的 `08b` 是全量连跑末段撞免费额度限流：429 重试（20s+45s）耗尽后模型响应降级，
   > 行为断言如实变红（丢事实 + 编数字都被抓到）。这不是误报，单跑即过（已实测）。
...
│   ├── 00、05、06a、07a、08a、10b、
│   │   11a、11b、16、17、18     #   不需要 Key（离线可跑，共 11 个文件）
│   ├── 其余 11 个              #   需要一个模型 Key
