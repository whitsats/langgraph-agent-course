...
| 10 | [MCP 与能力扩展（按需）](10-MCP与能力扩展.md) | `10_mcp_docs_server.py`、`10b_supervisor_lab.py` | 0.5–2 天 |

### 第四部分：做成能交付的东西

| 章 | 标题 | 配套例子 | 大概用时 |
|---|---|---|---|
| 11 | [工程化：评测、trace、上下文、安全](11-工程化.md) ★ | `11a`–`11d` 四个 lab + `run_all.py`（当回归用） | 2 天 |
...
| 19 | [附录F：精读生产级 Agent 项目](19-附录F-精读生产级Agent项目.md) | 外部仓库精读（无自带例子，产出 spec + 偏差清单） | **可选** · 约 1 周 |
...
> 2026-09-24 增补：离线组收编第 16 章安全实验（`16_agent_security_lab.py`）、第 17 章规划实验（`17_plan_and_verify_lab.py`）与第 18 章交接实验（`18_handoff_contract_lab.py`），`--offline` 现为 **8/8 通过**；另加 `mutation_check.py`——把 08a / 16 / 17 / 18 的关键不变量逐个改坏，验证断言真的会红，首跑 **15/15 被杀死**（第一次跑挂了两条"检查互相遮挡"，第二次暴露类型层没有用例，均已修），同日扩到 **20 条**（补 08a 租户隔离、17 幽灵工具、18 夹带过程 / 超长 / 新鲜性，并加 `compile()` 前置校验——语法错误的变异按"配置问题"报告，不再把崩溃误记成"杀死"），20/20 被杀死。另外三个实验室 + 08a 的检查已从"只打印"改成**真断言**（失败即非 0 退出，共 6 + 9 + 21 条，08a 另有 4 条），`run_all.py --offline` 的 ✅ 现在是真的 ✅；上表仍是 2026-09-23 那次全量实跑的存档。
>
> 2026-09-25 增补：第 10 / 11 章补 5 个 lab——`10b_supervisor_lab.py`（派活 / 扇出 / 失败隔离）、`11a_eval_lab.py`（评测集 + 回归对比）、`11b_context_budget_lab.py`（上下文预算三策略对比）、`11c_stream_frontend.py`（FastAPI 流式 + 最小前端）、`11d_cost_ledger.py`（token 成本账单），示例总数 17 → **22**（离线 11 + 在线 11）；变异表扩到 **23 条**（10b / 11a / 11b 各一条），23/23 被杀死。实测：离线 **11/11**、在线 **10/11**——`08b` 在全量连跑末段撞免费额度限流，重试耗尽后模型响应降级、行为断言如实变红（单跑即过，详见 `examples/README.md` 的"验证状态"）。
