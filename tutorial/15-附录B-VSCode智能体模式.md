...
### 阶段 2.5：把浏览器当执行环境（API → DOM → 视觉）

刚才装的 Playwright 不是普通工具，它是一整类能力的入口。**给智能体接"执行环境"有三层，能低不高**：

| 层 | 怎么做 | 成本 | 什么时候需要 |
|---|---|---|---|
| **API 层** | 直接调 HTTP 接口 | 最低 | 有公开 API 就永远优先这层 |
| **DOM 自动化层** | Playwright MCP：结构化地找元素、点击、填表、读页面 | 中 | 没有 API 的内部系统、表单流程 |
| **视觉操作层**（computer-use 类） | 截图 → 模型看图 → 定位鼠标键盘动作 | 最高 | 只有像素可看（桌面应用、验证码、Canvas） |

判断法则一句话：**下一层的确定性永远比上一层的生成性可靠**——能用 API 别用 DOM，能用 DOM 别用截图。视觉层是最后手段，不是展示品。

VS Code 里两者都已内置可用：DOM 层就是刚装的 Playwright MCP；视觉层对应 VS Code 的内置浏览器工具（agent 开你的本地应用、看渲染结果、检查控制台报错）——这正是第 13 行能力矩阵"浏览器 / computer-use"在编辑器里的形态。让 agent 验证前端需求时，直接说"打开应用走一遍登录，截图给我看结果"。
